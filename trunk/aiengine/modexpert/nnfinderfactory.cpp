			...
			for( int z2 = 10; z2--; )
				{
					...
					vi[1] = rnd.getRandomFloat();
					int clusterID = ( vi[0] < 0 || vi[1] < 0 )? 0 : 1;
					addSample( &samplesValidate , vi , clusterID );
				}
				...
				for( int kTrainCount = 50; kTrainCount <= 100; kTrainCount += 10 )
					{
						...
						for( int z1 = kTrainCount; z1--; )
							{
								...
								vi[1] = rnd.getRandomFloat();
								int clusterID = ( vi[0] < 0 || vi[1] < 0 )? 0 : 1;
								addSample( &samplesLearn , vi , clusterID );
							}
						...
						// factory data
						NNSample *fe = addFactoryStatSample();
...
void NNFinderFactory::testFactory()
{
