	...
	for( ; *l_format; l_format++ )
		{
			...
			l_mod = 0;
			if( !rfc_wstr_ncmp( (const short unsigned int*)l_format , (const short unsigned int*)RFC_WINT64FMTBASE , rfc_wstr_len( (const short unsigned int*)RFC_WINT64FMTBASE ) ) )
				{
					l_format += rfc_wstr_len((const short unsigned int*) RFC_WINT64FMTBASE );
					l_mod = 1;
				}
			else
				{
