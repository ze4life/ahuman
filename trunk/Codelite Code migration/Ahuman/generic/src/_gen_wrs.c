...
{
	return( LoadAccelerators( (HINSTANCE__*)p_inst , p_name ) );
}
	...
	rfc_dlp_res_accel( rfc_dllpool *p_dlp , int p_lib , const char *p_name )
{
	...
	return( LoadAccelerators( (HINSTANCE__*)l_lib -> s_lib , p_name ) );
}
...
RFC_HND
	rfc_res_image( RFC_HND p_inst , 
		...
		UINT p_load )
{
	return( LoadImage( (HINSTANCE__*)p_inst , p_name , p_type , p_cx , p_cy , p_load ) );
}
		...
		UINT p_load )
{
	...
	return( LoadImage( (HINSTANCE__*)l_lib -> s_lib , p_name , p_type , p_cx , p_cy , p_load ) );
}
...
{
	return( LoadMenu((HINSTANCE__*) p_inst , p_name ) );
}
	...
	rfc_dlp_res_menu( rfc_dllpool *p_dlp , int p_lib , const char *p_name )
{
	...
	return( LoadMenu( (HINSTANCE__*)l_lib -> s_lib , p_name ) );
}
	...
	rfc_res_string( RFC_HND p_inst , int p_id )
{
	...
	*l_buf = 0;
	...
	l_len = LoadString( (HINSTANCE__*)p_inst , p_id , l_buf , 1024 );
	l_buf[ l_len ] = 0;
	...
	rfc_dlp_res_string( rfc_dllpool *p_dlp , int p_lib , int p_id )
{
	...
	*l_buf = 0;
	...
	l_len = LoadString( (HINSTANCE__*)l_lib -> s_lib , p_id , l_buf , 1024 );
	l_buf[ l_len ] = 0;
	...
	rfc_res_dialog( RFC_HND p_inst , const char *p_name )
{
	...
	l_res = FindResource( (HINSTANCE__*)p_inst , p_name , RT_DIALOG );
	if( l_res == NULL )
		return( NULL );

	return( LoadResource( (HINSTANCE__*)p_inst , l_res ) );
}
	...
	rfc_dlp_res_dialog( rfc_dllpool *p_dlp , int p_lib , const char *p_name )
{
	...
	l_res = FindResource( (HINSTANCE__*)l_lib -> s_lib , p_name , RT_DIALOG );
	if( l_res == NULL )
		return( NULL );

	return( LoadResource( (HINSTANCE__*)l_lib -> s_lib , l_res ) );
}
	...
	rfc_res_custom( RFC_HND p_inst , const char *p_name , const char *p_type )
{
	...
	l_res = FindResource((HINSTANCE__*) p_inst , p_name , p_type );
	if( l_res == NULL )
		return( NULL );
