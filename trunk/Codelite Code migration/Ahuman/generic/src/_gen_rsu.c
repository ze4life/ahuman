	...
	rfc_ustr_create( const rfc_uchar *p_s , int p_len , int p_allocate )
{
	...
	*( int * )l_p = l_size;
	...
	if( p_len )
		strcpy( (char*)l_p , (const char*)p_s );
	else
		*l_p = 0;
	return( (rfc_uchar*)l_p );
}
	...
	rfc_ustr_allocate( rfc_ustring p_str , int p_size )
{
	...
	/* reallocate */
	p_str -= sizeof( int );
	p_str = ( rfc_uchar * )realloc( p_str , p_size * 3 + sizeof( int ) + 1 );
	*( int * )p_str = p_size * 3;
	p_str += sizeof( int );
	...
	rfc_ustr_append( rfc_ustring p_str , int p_pos , const rfc_uchar *p_s , int p_slen )
{
	...
	/* check allocation */
	p_str = (rfc_uchar*)rfc_str_allocate( (char*)p_str , l_len + p_slen );

	/* shift tail */
	l_size = ( p_str )? strlen( (const char*)p_str ) : 0;
	...
	rfc_ustr_format_va( rfc_ustring p_str , const rfc_uchar *p_format , va_list p_va )
{
	...
	return( (rfc_uchar*)rfc_str_format_va( ( rfc_string )p_str , ( const char * )p_format , p_va ) );
}
